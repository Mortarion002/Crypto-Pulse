...
export default function WatchlistPage() {
  const { savedCoins, removeCoin } = useWatchlist()
  const { coins, loading, error } = useMarketData()

  const watchedCoins = coins.filter((c) => savedCoins.includes(c.displaySymbol))
      ...
      <main className="mx-auto flex max-w-[900px] flex-col gap-8 px-6 py-12">
        <section className="flex items-center justify-between gap-4">
          ...
          <span className="rounded-full border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.05)] px-3 py-1.5 text-[12px] font-semibold text-[#8A8A9A]">
            {savedCoins.length} coin{savedCoins.length === 1 ? '' : 's'}
          </span>
        </section>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#8A8A9A]">{coin.displaySymbol}</p>
                      </div>
                    </div>
                    ...
                    <div
                      className="text-right text-sm text-[#8A8A9A] transition-colors hover:text-[#FF4D4D]"
                      onClick={(e) => {
                        e.preventDefault()
                        e.stopPropagation()
                        removeCoin(coin.displaySymbol)
                      }}
                    >
                      Remove
                    </div>

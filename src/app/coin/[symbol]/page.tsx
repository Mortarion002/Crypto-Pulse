'use client'

import Link from 'next/link'
import { useParams } from 'next/navigation'
import { motion } from 'framer-motion'

import type { Sentiment } from '@/features/market/types'
import Card from '@/shared/components/Card'
import TrendBadge from '@/shared/components/TrendBadge'
import { getCoinBadgeClass } from '@/shared/utils/coinColors'
import { formatChange, formatPrice } from '@/shared/utils/format'

interface CoinDetail {
  name: string
  symbol: string
  color: string
  price: number
  priceChange: number
  sentiment: Sentiment
  high24h: number
  low24h: number
  volume: number
  bullPercent: number
  insight: string
}

const MOCK_COIN_DETAILS: Record<string, CoinDetail> = {
  BTC: {
    name: 'Bitcoin',
    symbol: 'BTC',
    color: '#F7931A',
    price: 67420.5,
    priceChange: 3.24,
    sentiment: 'Bullish',
    high24h: 68100,
    low24h: 65200,
    volume: 2100000000,
    bullPercent: 68,
    insight: 'Strong buying pressure in last 4H',
  },
  ETH: {
    name: 'Ethereum',
    symbol: 'ETH',
    color: '#627EEA',
    price: 3842.18,
    priceChange: 2.17,
    sentiment: 'Bullish',
    high24h: 3925,
    low24h: 3710,
    volume: 1320000000,
    bullPercent: 61,
    insight: 'Breakout continuation holding above key support',
  },
  SOL: {
    name: 'Solana',
    symbol: 'SOL',
    color: '#14F195',
    price: 178.65,
    priceChange: 5.48,
    sentiment: 'Bullish',
    high24h: 182.2,
    low24h: 166.7,
    volume: 820000000,
    bullPercent: 72,
    insight: 'Momentum accelerating after strong session close',
  },
  BNB: {
    name: 'BNB',
    symbol: 'BNB',
    color: '#F3BA2F',
    price: 612.4,
    priceChange: 0.14,
    sentiment: 'Neutral',
    high24h: 618.4,
    low24h: 606.1,
    volume: 475000000,
    bullPercent: 51,
    insight: 'Price compressing into a neutral consolidation band',
  },
  ADA: {
    name: 'Cardano',
    symbol: 'ADA',
    color: '#0033AD',
    price: 0.6782,
    priceChange: -1.82,
    sentiment: 'Bearish',
    high24h: 0.7041,
    low24h: 0.6642,
    volume: 190000000,
    bullPercent: 38,
    insight: 'Buyers fading as sellers defend intraday rallies',
  },
  XRP: {
    name: 'XRP',
    symbol: 'XRP',
    color: '#00AAE4',
    price: 0.5421,
    priceChange: 4.11,
    sentiment: 'Bullish',
    high24h: 0.5562,
    low24h: 0.5218,
    volume: 265000000,
    bullPercent: 64,
    insight: 'Fresh inflows supporting a clean upside reversal',
  },
}

function formatVolume(volume: number): string {
  return (
    '$' +
    new Intl.NumberFormat('en-US', {
      notation: 'compact',
      maximumFractionDigits: 1,
    }).format(volume)
  )
}

function getTone(sentiment: Sentiment): string {
  if (sentiment === 'Bullish') return 'text-[#00FF85]'
  if (sentiment === 'Bearish') return 'text-[#FF4D4D]'
  return 'text-[#9B6DFF]'
}

function getArrow(value: number): string {
  return value >= 0 ? '↑' : '↓'
}

function BoltIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-5 w-5 fill-current" aria-hidden="true">
      <path d="M13.5 2 6 13h4.7L9.8 22 18 10.8h-4.6L13.5 2Z" />
    </svg>
  )
}

export default function CoinDetailPage() {
  const params = useParams<{ symbol: string }>()
  const requestedSymbol = params.symbol?.toUpperCase() ?? 'BTC'
  const coin = MOCK_COIN_DETAILS[requestedSymbol] ?? MOCK_COIN_DETAILS.BTC
  const bearPercent = 100 - coin.bullPercent

  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.3 }}
    >
      <main className="mx-auto flex max-w-[1200px] flex-col gap-8 px-6 py-8">
        <div className="text-sm text-[#8A8A9A]">
          <Link href="/" className="transition-colors hover:text-white">
            ← Market
          </Link>{' '}
          / <span className="text-white">{coin.name}</span> ({coin.symbol})
        </div>

        <div className="rounded-2xl bg-[radial-gradient(650px_220px_at_0%_30%,rgba(0,255,133,0.12),transparent_72%)]">
          <Card className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="flex max-w-2xl flex-col gap-5">
              <div className="flex items-center gap-4">
                <div
                  className={`flex h-14 w-14 items-center justify-center rounded-full text-xl font-bold text-white ${getCoinBadgeClass(
                    coin.symbol
                  )}`}
                >
                  {coin.symbol.charAt(0)}
                </div>
                <div className="flex items-center gap-3">
                  <h1 className="text-3xl font-bold text-white">{coin.name}</h1>
                  <span className="rounded-full border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.04)] px-3 py-1 text-xs font-medium uppercase tracking-[0.22em] text-[#8A8A9A]">
                    {coin.symbol}
                  </span>
                </div>
              </div>

              <div className="flex flex-col gap-3">
                <p className="font-mono text-4xl font-extrabold text-white sm:text-5xl">
                  {formatPrice(coin.price)}
                </p>
                <div className="flex flex-wrap items-center gap-3">
                  <span className={`text-xl font-semibold ${getTone(coin.sentiment)}`}>
                    {formatChange(coin.priceChange)} {getArrow(coin.priceChange)}
                  </span>
                  <TrendBadge sentiment={coin.sentiment} />
                </div>
              </div>
            </div>

            <div className="grid w-full gap-3 sm:grid-cols-3 lg:max-w-[420px]">
              {[
                { label: '24H High', value: formatPrice(coin.high24h) },
                { label: '24H Low', value: formatPrice(coin.low24h) },
                { label: 'Volume', value: formatVolume(coin.volume) },
              ].map((stat) => (
                <div
                  key={stat.label}
                  className="rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)] p-4"
                >
                  <p className="text-[11px] uppercase tracking-widest text-[#8A8A9A]">
                    {stat.label}
                  </p>
                  <p className="mt-3 text-lg font-bold text-white">{stat.value}</p>
                </div>
              ))}
            </div>
          </Card>
        </div>

        <Card className="flex flex-col gap-5">
          <div className="flex items-center justify-between">
            <p className="text-[13px] uppercase tracking-[0.22em] text-[#8A8A9A]">
              Price Chart
            </p>
            <p className="text-sm text-[#8A8A9A]">1D range</p>
          </div>

          <div className="flex h-[300px] items-center justify-center rounded-lg border border-[rgba(255,255,255,0.06)] text-sm text-[#8A8A9A]">
            Chart will load here
          </div>

          <div className="flex flex-wrap gap-2">
            {['1H', '4H', '1D', '1W'].map((range) => (
              <button
                key={range}
                type="button"
                className={
                  range === '1D'
                    ? 'rounded-full bg-[#00FF85] px-4 py-1.5 text-xs font-semibold text-[#0B0B0F]'
                    : 'rounded-full border border-[rgba(255,255,255,0.12)] px-4 py-1.5 text-xs text-[#8A8A9A]'
                }
              >
                {range}
              </button>
            ))}
          </div>
        </Card>

        <section className="grid grid-cols-1 gap-5 lg:grid-cols-[2fr_1fr]">
          <Card className="flex flex-col gap-5">
            <div className="flex items-center justify-between">
              <p className="text-[13px] uppercase tracking-[0.22em] text-[#8A8A9A]">
                Sentiment Analysis
              </p>
              <p className="text-sm text-[#8A8A9A]">24h window</p>
            </div>

            <div className="overflow-hidden rounded-full bg-[rgba(255,255,255,0.06)]">
              <div className="flex h-[10px]">
                <div className="w-[68%] bg-[#00FF85]" />
                <div className="w-[32%] bg-[#FF4D4D]" />
              </div>
            </div>

            <div className="flex items-center justify-between text-sm font-semibold">
              <p className="text-[#00FF85]">{coin.bullPercent}% Bullish</p>
              <p className="text-[#FF4D4D]">{bearPercent}% Bearish</p>
            </div>

            <div className="flex items-center gap-4 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)] p-4">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[rgba(0,255,133,0.1)] text-[#00FF85]">
                <BoltIcon />
              </div>
              <p className="text-sm text-white">{coin.insight}</p>
            </div>
          </Card>

          <Card className="flex flex-col items-center justify-center gap-4 text-center">
            <button
              type="button"
              className="w-full rounded-xl border border-[#00FF85] px-4 py-3 text-sm font-semibold text-[#00FF85] transition-colors hover:bg-[rgba(0,255,133,0.08)]"
            >
              Add to Watchlist
            </button>
            <p className="text-[12px] text-[#8A8A9A]">
              Get alerts when sentiment flips
            </p>
          </Card>
        </section>
      </main>
    </motion.div>
  )
}
